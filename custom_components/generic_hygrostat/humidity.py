...
#from homeassistant.components.humidity import PLATFORM_SCHEMA, HumidityDevice
#from homeassistant.components.humidity.const import (
from custom_components.humidity import PLATFORM_SCHEMA, HumidityDevice
from custom_components.humidity.const import (
    ATTR_PRESET_MODE,
    ATTR_HUMIDITY,
...
CONF_INITIAL_HUMIDIFIER_MODE = "initial_humidifier_mode"
...
PLATFORM_SCHEMA = PLATFORM_SCHEMA.extend(
    {
        ...
        vol.Optional(CONF_INITIAL_HUMIDIFIER_MODE): vol.In(
            [HUMIDIFIER_MODE_DRY, HUMIDIFIER_MODE_HUMIDIFY, HUMIDIFIER_MODE_OFF]
        ),
...
async def async_setup_platform(hass, config, async_add_entities, discovery_info=None):
    ...
    initial_humidifier_mode = config.get(CONF_INITIAL_HUMIDIFIER_MODE)
...
class GenericHygrostat(HumidityDevice, RestoreEntity):
    ...
    def __init__(
        ...
        self._humidifier_mode = initial_humidifier_mode
        self._saved_target_humidity = target_humidity or away_humidity
        if self.dry_mode:
            self._humidifier_list = [HUMIDIFIER_MODE_DRY, HUMIDIFIER_MODE_OFF]
        else:
            self._humidifier_list = [HUMIDIFIER_MODE_HUMIDIFY, HUMIDIFIER_MODE_OFF]
        self._active = False
    ...
    async def async_added_to_hass(self):
        ...
        if old_state is not None:
            ...
            if old_state.attributes.get(ATTR_PRESET_MODE) == PRESET_AWAY:
                self._is_away = True
            if not self._humidifier_mode and old_state.state:
                self._humidifier_mode = old_state.state
        ...
        if not self._humidifier_mode:
            self._humidifier_mode = HUMIDIFIER_MODE_OFF
    ...
    def humidifier_action(self):
        ...
        """
        if self._humidifier_mode == HUMIDIFIER_MODE_OFF:
    ...
    async def async_set_humidifier_mode(self, humidifier_mode):
        """Set humidifier mode."""
        if humidifier_mode == HUMIDIFIER_MODE_HUMIDIFY:
            self._humidifier_mode = HUMIDIFIER_MODE_HUMIDIFY
            await self._async_control_humidifying(force=True)
        elif humidifier_mode == HUMIDIFIER_MODE_DRY:
            self._humidifier_mode = HUMIDIFIER_MODE_DRY
            await self._async_control_humidifying(force=True)
        elif humidifier_mode == HUMIDIFIER_MODE_OFF:
            self._humidifier_mode = HUMIDIFIER_MODE_OFF
            if self._is_device_active:
                await self._async_humidifier_turn_off()
        else:
            _LOGGER.error("Unrecognized humidifier mode: %s", humidifier_mode)
            return
    ...
    async def _async_control_humidifying(self, time=None, force=False):
        ...
        async with self._humidity_lock:
            ...
            if not self._active or self._humidifier_mode == HUMIDIFIER_MODE_OFF:
                return
            ...
            if not force and time is None:
                ...
                if self.min_cycle_duration:
                    if self._is_device_active:
                        ...
                        current_state = HUMIDIFIER_MODE_OFF
