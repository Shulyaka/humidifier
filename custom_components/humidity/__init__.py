...
async def async_setup(hass: HomeAssistantType, config: ConfigType) -> bool:
    ...
    component.async_register_entity_service(
        SERVICE_SET_HUMIDIFIER_MODE, SET_HUMIDIFIER_MODE_SCHEMA, "async_set_humidifier_mode"
    )
    component.async_register_entity_service(
...
class HumidityDevice(Entity):
    ...
    async def async_set_humidifier_mode(self, humidifier_mode: str) -> None:
        """Set new target humidifier mode."""
        await self.hass.async_add_executor_job(self.set_humidifier_mode, humidifier_mode)
    ...
    async def async_turn_on(self) -> None:
        ...
        # Fake turn on
        for mode in (HUMIDIFIER_MODE_HUMIDIFY_DRY, HUMIDIFIER_MODE_HUMIDIFY, HUMIDIFIER_MODE_DRY):
